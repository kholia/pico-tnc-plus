...
float AHTxx::readHumidity(bool readAHT)
{
  if (readAHT == AHTXX_FORCE_READ_DATA) {
    _readMeasurement(); //force to read data via I2C & update "_rawData[]" buffer
  }
...
bool AHTxx::softReset()
{
  ...
  if (Wire.endTransmission(true) != 0) {
    return false; //collision on I2C bus, sensor didn't return ACK
  }
...
void AHTxx::_readMeasurement()
{
  ...
  if ((_sensorType == AHT2x_SENSOR) && (_checkCRC8() != true)) {
    _status = AHTXX_CRC8_ERROR; //update status byte
  }
}
...
bool AHTxx::_setInitializationRegister(uint8_t value)
{
  ...
  if   (_sensorType == AHT1x_SENSOR) {
    Wire.write(AHT1X_INIT_REG); //send initialization command, for AHT1x only
  }
  else                               {
    Wire.write(AHT2X_INIT_REG); //send initialization command, for AHT2x only
  }
...
uint8_t AHTxx::_readStatusRegister()
{
  ...
  if (Wire.available() == 1) {
    return Wire.read(); //read 1-byte from "wire.h" rxBuffer
  }
...
uint8_t AHTxx::_getBusy(bool readAHT)
{
  if (readAHT == AHTXX_FORCE_READ_DATA)                    //force to read data via I2C & update "_rawData[]" buffer
  {
    ...
    if (Wire.available() != 1) {
      return AHTXX_DATA_ERROR; //no reason to continue, "return" terminates the entire function & "break" just exits the loop
    }
  ...
  if   ((_rawData[0] & AHTXX_STATUS_CTRL_BUSY) == AHTXX_STATUS_CTRL_BUSY) {
    _status = AHTXX_BUSY_ERROR; //0x80=busy, 0x00=measurement completed
  }
  else                                                                    {
    _status = AHTXX_NO_ERROR;
  }
